import XCTest
@testable import Loosechange

@MainActor
final class LoosechangeTests: XCTestCase {
    var store: LoosechangeStore!

    override func setUp() async throws {
        store = LoosechangeStore()
    }
    ...
    func testFreeLimitBlocksAddition() {
        store.isPro = false
        for i in 0..<(LoosechangeStore.freeLimit + 5) {
            store.add(title: "Filler \(i)", amount: 1.0)
        }
        XCTAssertEqual(store.entries.count, LoosechangeStore.freeLimit)
    }
    ...
    func testCanAddMoreWhenPro() {
        ...
        XCTAssertGreaterThan(store.entries.count, LoosechangeStore.freeLimit)
    }
    ...
    func testFreeLimitAboveSeedCount() {
        XCTAssertGreaterThan(LoosechangeStore.freeLimit, 3)
    }
}
